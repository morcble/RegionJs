...
var REGION = {
		init:function(){
			...
			var dateCoord = curRegion.paraMap.get("dateCoord");//日期坐标
			var startDate = curRegion.paraMap.get("startDate");//结束日期
			var endDate = curRegion.paraMap.get("endDate");//结束日期
			...
			curRegion.dateCoordValue = moment(dateCoord,curRegion.dateFormat);
			curRegion.startDate = moment(startDate,curRegion.dateFormat);
			curRegion.endDate = moment(endDate,curRegion.dateFormat);

			if(!curRegion.dateCoordValue.isValid()){
				curRegion.dateCoordValue = moment();
			}
			
			curRegion.navStatus = {};//缓存顶部输入的年月
			curRegion.navStatus.year = curRegion.dateCoordValue.year();
			curRegion.navStatus.month = curRegion.dateCoordValue.month();
			//curRegion.navStatus.day = curRegion.dateCoordValue.date();
		},
		renderCalendar:function(){
			...
			//选择开始日期
			if(curRegion.startDate!=null&&curRegion.startDate.year()==curRegion.navStatus.year){
				if(curRegion.startDate.month()==curRegion.navStatus.month){
					calendarObj.find(".curMonth"+curRegion.startDate.date()).addClass("chosen-date");
				}
			}
			
			//选择结束日期
			if(curRegion.endDate!=null&&curRegion.endDate.year()==curRegion.navStatus.year){
				if(curRegion.endDate.month()==curRegion.navStatus.month){
					calendarObj.find(".curMonth"+curRegion.endDate.date()).addClass("chosen-date");
				}
			}
			
			//选择日期范围
			if(curRegion.startDate!=null&&curRegion.endDate!=null&&curRegion.startDate.isValid()&&curRegion.endDate.isValid())
				curRegion.selectRange(curRegion.startDate,curRegion.endDate);
			...
			calendarObj.find(".day").click(function(){
				...
				if(typeof curRegion.onDayClick==="function"){
					curRegion.onDayClick(dayCell,curRegion.getCellDate(dayCell));
				}
			});
			...
			//console.log(curRegion.getDate().toString())
		},
		...
		selectRange:function(startDt,endDt){//范围选择  从选中的日期到 endDtStr（符合dateRegion.dateFormat格式的日期）
			var curRegion = this;
			var startDtAsStr = startDt.format(curRegion.dateFormat);
			var endDtAsStr = endDt.format(curRegion.dateFormat);
			
			curRegion.find(".day").removeClass("selected");
			
			if(startDt==endDtAsStr)return;
			
			//保证startDt比endDt小
			if(startDtAsStr>endDtAsStr){
				var tmp = startDtAsStr;
				startDtAsStr = endDtAsStr;
				endDtAsStr = tmp;
			}
			
			curRegion.find(".day").each(function(){
				var dayCell = $(this);
				if(!dayCell.hasClass("notallowed")&&!dayCell.hasClass("nextmonth")&&!dayCell.hasClass("lastmonth")){
					var cellDate = curRegion.getCellDate(dayCell);
					if((cellDate.format(curRegion.dateFormat)>=startDtAsStr)&&(cellDate.format(curRegion.dateFormat)<=endDtAsStr)){
...
RegionUtil.ready(function(){
	...
	dateRegion.selectRange = REGION.selectRange;//范围选择
	
	dateRegion.dateCoordValue = null;//缓存日历
...
// 	dateRegion.onDayClick = function(date){//鼠标点击DAY
// 			alert(1)
// };
	dateRegion.onDayHover = function(dayCell,date){//鼠标移动到DAY
		/* console.log(this)
		console.log(dayCell)
		console.log(date.toString())
		console.log(this.getDate().toString()) */
		//dateRegion.selectRange(this.getDate(),date);
	};
