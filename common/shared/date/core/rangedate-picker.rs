...
var REGION = {
		afterRenderData:function(){
			...
			if(endDtMoment!=null&&endDtMoment.isValid()){
				if((endDtMoment.year()==startDtMoment.year())&&(endDtMoment.month()==startDtMoment.month())){
					dateCoordMoment.month(dateCoordMoment.month()+1);
				}
				else{
					dateCoordMoment = endDtMoment;
				}
			}
			else{
				dateCoordMoment.month(dateCoordMoment.month()+1);
			...
			var paraMap2 = new HashMap();
			paraMap2.put("dateFormat",dateFormat);
			paraMap2.put("dateCoord",dateCoordMoment.format(dateFormat));
			paraMap2.put("startDate",startDt);
			paraMap2.put("endDate",endDt);
			paraMap2.put("minDate",minDate);
			paraMap2.put("maxDate",maxDate);
			paraMap2.put("calStyle",calStyle);
			if(exclude!=null)paraMap2.put("exclude",exclude);
			
			var endLoadPromise = RS.loadRegion(curRegion.find(".endTimeHolder"),RS.appendParaMapToUrl("common/shared/date/core/rangedate.rs",paraMap2));
			endLoadPromise.done(function(endDateRegion){
				curRegion.endDateRegion = endDateRegion;
				
				endDateRegion.onDayClick=function(dayCell,cellDate){
					REGION.handleRangeClick(curRegion,dayCell,cellDate);
				}
			})
			
			return startLoadPromise;
		},
		handleRangeClick:function(curRegion,dayCell,cellDate){
			var startDateRegion = curRegion.startDateRegion;
			var endDateRegion = curRegion.endDateRegion;
			
			if(curRegion.selecting == false){
				curRegion.selecting = true;
				startDateRegion.startDate = cellDate;
				startDateRegion.endDate = null;
				endDateRegion.startDate = cellDate;
				endDateRegion.endDate = null;
				
				startDateRegion.renderCalendar();
				endDateRegion.renderCalendar();
				
				startDateRegion.onDayHover = function(dayCell,date){
					startDateRegion.endDate = date;
					endDateRegion.endDate = date;
					
					startDateRegion.selectRange(startDateRegion.startDate,startDateRegion.endDate);
					endDateRegion.selectRange(startDateRegion.startDate,startDateRegion.endDate);
				};
				
				endDateRegion.onDayHover = function(dayCell,date){
					startDateRegion.endDate = date;
					endDateRegion.endDate = date;
				...
				delete endDateRegion.onDayHover;
				
				startDateRegion.endDate = cellDate;
				endDateRegion.endDate = cellDate;
