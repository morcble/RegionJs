	 		...
	 		def.done(function(loadedRegion){
	 			...
	 			for(var i = 0 ; i <screenNodes.length ;i++){
	 				...
	 				if(screenNodes[i].type=="rs"){
	 					...
	 					screensInfo.push({'name':screenName,'screenRegion':'common/shared/index/screenStyle/screenCommon.rs','useCache':true });
	 				}
	 			}
...
// 	 			];
	 			loadedRegion.onSwitch = function(screenName,screenRegion){//屏幕切换之后需要更新菜单
	 				...
	 				adminMainRegion.headerRegion.moduleRegion.find(".nav-list>li").each(function(){
	 					if($(this).hasClass("nav_"+screenName)){
