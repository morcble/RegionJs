...
<div id="REGION" class="hidden REGION_overlay">
	<div class="tree" style="height:100%;padding-bottom:0.625rem; padding-top: 0.25rem">
...
var REGION = {
		renderContent:function(){
			...
			this.renderMenu = REGION.renderMenu.bind(this);//兼容旧的
			...
			this.getNodeDataById = REGION.getNodeDataById.bind(this);
			
			var curRegion = this;
			curRegion.setTreeData = function(treeDataPara){
				curRegion.treeData = treeDataPara;
				curRegion.renderTree(treeDataPara);
			}
		...
		restoreMenuNode:function(menuNode,curRegion){//还原菜单
			...
			branchObj.find(".node-item").css("display","");
		...
		minimizeMenuNode:function(menuNode,curRegion){//缩小菜单
			var menuNodeObj = $(menuNode);
			var branchObj = menuNodeObj.find(".branch");
			branchObj.find(".node-item").css("display","none");
		...
		defaultNodeRender:function(nodeData){
			return "<span class='node-item'>"+nodeData.label+"</span>";
		},
			...
			if(childs!=null&&childs.length!=0){//绘制有child的节点
				...
				if(nodeData.depth>1){
					if(this.showDots=="true")
						nodeHtml+="<span class='lined' style='padding-left:"+((1+paddingLeft*(nodeData.depth-1)) /fontSize)+"rem'></span>";
					else	
				...
				nodeHtml+="<div node-id='"+nodeData.id+"' onclick='REGION.selectNode(\""+nodeData.id+"\")' class='node_"+nodeData.id+" non-selected "+nodeClass+"'>";
				if(this.showDots=="true")nodeHtml+='<div class="crossline-v" style="left:'+((2+paddingLeft*(nodeData.depth-1)) / fontSize)+'rem"></div>';
				if(nodeData.depth>1){
					nodeHtml+="<div style='background:white' class='"+currentNodeType+"'>";
				}
				else{
		...
		getNodeDataById:function(nodeId){
			var curRegion = this;
			return curRegion.nodeMap.get(nodeId);
		},
		resolveTreeData:function(curRegion,nodeDataList){//把节点list组装成tree  startDepth:开始构造树的depth,之前的节点全部忽略
			...
			for(var i=0;i<nodeDataList.length;i++){
				tmpParentNodeData = nodeMap.get(nodeDataList[i].pid);
			...
			//数据处理
			//子节点排序
			REGION.sortChilds(rootNode,curRegion.orderBySeqAsc);//parentId为0的节点排序
			for(var i=0;i<nodeDataList.length;i++){
				//计算expand
				...
				//子节点排序
				REGION.sortChilds(nodeDataList[i],curRegion.orderBySeqAsc);
		...
		sortChilds:function(nodeData,orderBySeqAsc){
			var childs = nodeData.childs;
			if(childs==null||childs.length==0)return;
			console.log(orderBySeqAsc)
			for(var i = 0 ; i<childs.length-1 ;i++){
				for(var j = i+1 ; j<childs.length ;j++){
					if(orderBySeqAsc==true){
		...
		renderTree:function(treeListData){
			this.treeData = treeListData;
			this.renderMenu(treeListData);
		},
		renderMenu:function(treeListData,orderBySeqAsc){//绘制整个树节点   
			if(orderBySeqAsc!=true)orderBySeqAsc=false; //默认按seqNo降序排列
			this.orderBySeqAsc=orderBySeqAsc;
			...
			curRegion.find(".icon-holder>.fa").mouseover(function(){
				var msg = $(this).closest(".branch").find(".node-item").text();
		...
		showMenu:function(currentRegion){
			...
			if(menuItems!=null){
				for(var i = 0 ; i <menuItems.length ; i++){
					itemHtml = menuHtml.replace("part1",menuItems[i].onclick+"(event,"+i+",'"+menuItems[i].label.trim()+"')");
		...
		addNode:function(nodeData){//添加一个节点并渲染
			var parentNodeData = this.nodeMap.get(nodeData.pid);
			if(parentNodeData==null){
				RegionUtil.alert("Parent node not found with pid:"+nodeData.pid);
				return;
			}
			...
			REGION.sortChilds(parentNodeData,this.orderBySeqAsc);
		...
		refreshNode:function(nodeData){//刷新某个节点的数据，并渲染
			if(nodeData.id=="0"){//刷新根节点
				...
				var parentNodeData = this.nodeMap.get(nodeData.pid);				
				var existNodeData = this.nodeMap.get(nodeData.id);
				if(existNodeData==null)return;
				for(x in nodeData){
					if(nodeData[x]!=null){
						existNodeData[x] = nodeData[x];
					}
				}
			
				var replaceHtml = REGION.getNodeFullHtml.call(this,existNodeData,existNodeData.parent);
				var replaced = $(replaceHtml);
				RegionUtil.addRegionUniqueId(replaced[0],this.regionId);
				var nodeObj = this.find(".node_"+existNodeData.id);
