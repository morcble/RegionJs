...
var REGION = {
		renderContent:function(){
			...
			this.showDots = this.paraMap.get("showDots");//accordion = 'true' 手风琴模式
			this.multipleSelect = this.paraMap.get("multipleSelect");
			if(this.multipleSelect!="false")this.multipleSelect="true";
			...
			this.getSelected = REGION.getSelected.bind(this);
		...
		getSelected:function(){
			var result = [];
			var curRegion = this;
			curRegion.find("input[type=checkbox]").each(function(){
				if($(this).prop("checked")){
					var id = $(this).val();
					var node = curRegion.nodeMap.get(id);
					if(node!=null){
						result.push(node);
					}
				}
		...
		getNodeFullHtml:function(nodeData,parentNodeData){//拼接node的整体html
			...
			if(childs!=null&&childs.length!=0){//绘制有child的节点
				...
				let curRegion = this;
				if(curRegion.multipleSelect=="false"){//非多选的时候，非子节点不能加checkbox
					
				}
				else{
					nodeHtml+='<input class="checkAll checkbox'+nodeData.id+'" type="checkbox" value="'+nodeData.id+'"/>';
				}
				...
				if(nodeData.icon!=null)nodeHtml+="<span class='icon-holder'><i class='fa  "+nodeData.icon+"'></i></span>";
				nodeHtml+='<input class="checkItem checkbox'+nodeData.id+'" type="checkbox" value="'+nodeData.id+'"/>';
				nodeHtml+=this.nodeRender.call(this,nodeData);
				nodeHtml+="</div>";
				nodeHtml+="</div>";
				nodeHtml+="</div>";
		...
		renderTree:function(treeListData){
			...
			curRegion.find(".checkAll").click(function(){
				var event = window.event || arguments.callee.caller.arguments[0];
				event.stopPropagation();
				
				if(curRegion.onChecked!=null&&typeof(curRegion.onChecked)==="function" ){
					curRegion.onChecked(this);
				}
				
				var checkBox = $(this);
				var checked = checkBox.prop("checked");
				if(curRegion.multipleSelect=="true"){
					checkBox.closest(".pad").find(".childs").find("input[type=checkbox]").prop("checked",checked);
				}
				else{
					curRegion.find("input[type=checkbox]").prop("checked",false);
					checkBox.prop("checked",checked);
				}
			});
			
			curRegion.find(".checkItem").click(function(){
				var event = window.event || arguments.callee.caller.arguments[0];
				event.stopPropagation();
				...
				if(curRegion.multipleSelect=="false"){
					var checkBox = $(this);
					var checked = checkBox.prop("checked");
					curRegion.find("input[type=checkbox]").prop("checked",false);
					checkBox.prop("checked",checked);
			...
			for(var i=0;i<treeListData.length;i++){
				var tmpNode = treeListData[i];
				if(tmpNode.checked){
					curRegion.find(".checkbox"+tmpNode.id).prop("checked",true);
				}
			}
		...
		selectNode:function(nodeId){
			...
			if(event==null){//非点击触发
				...
				if(nodeData.childs==null){//如果是叶子节点，需要修改checkbox状态
					var checkBox = curRegion.find(".checkbox"+nodeId);
