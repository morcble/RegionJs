...
var REGION = {
		...
		loadScreen:function(screenName,paraMap,pushHistory,direction){//加载screen,pushHistory 默认为true,记录跳转了历史记录
			...
			if(screenInfo==null){
				...
				if(curRegion.screensInfo!=null && curRegion.screensInfo.length>0){
					console.error("Redirect to default screen:"+curRegion.screensInfo[0].name);
					return curRegion.loadScreen(curRegion.screensInfo[0].name,paraMap,pushHistory,direction);
				}
				else 
					return;
			}
			
			var paraJson = RegionUtil.appendParaMapToUrl("",paraMapForCache);//屏幕缓存的标识
