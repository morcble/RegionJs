...
( function() {
	var imageDialog = function( editor, dialogType ) {
			...
			var numbering = function( id ) {
				...
				browseFileId = numbering( 'browseFile' );
			...
			return {
				...
				onShow: function() {
					...
					if ( this.imageEditMode ) {
						...
						// Fill out all fields.
						editor.imageAlt = this.imageElement.getAttribute("alt");
				...
				onOk: function() {
					...
					// Create a new image.
						...
						//change for regionsoft
						this.imageElement.setAttribute("alt",document.getElementById(browseFileId).getAttribute("alt"));
					}
				...
				contents: [ {
					id: 'info',
					label: '图片信息',//editor.lang.image.infoTab,
					accessKey: 'I',
					elements: [ {
						type: 'vbox',
						padding: 0,
						children: [ {
								...
								onChange: function() {
									...
									if ( newUrl.length > 0 ) {
										...
										if ( dialog.preview ) {
											...
											dialog.preview.setAttribute( 'alt', editor.imageAlt);//regionsoft change
											updatePreview( dialog );
										}
									}
									// Dont show preview if no URL given.
							...
							},
							{
								...
								onClick: function(){
									document.getElementById(browseFileId).click();
								}
							},
							{//change for regionsoft
								type: 'html',
								...
								align: 'center',
								hidden: true,
								html: '<input  id="'+browseFileId+'"type="file" name="browseFile" size="">',
								onLoad: function() {
									var browseFile = CKEDITOR.document.getById(browseFileId );
									var dialog = this.getDialog();
									browseFile.on('change', function() {
										var urlInput = dialog.getContentElement( 'info', 'txtUrl' );

										var file = event.target.files[0];
										CoverPlugin.loadingStart();
										var completeDef = ObjectSystemUtil.uploadFile(file,"bizpic",Config.fileVendor);
										completeDef.done(function(serverData){
										    	console.log("finished");
										    	console.log(serverData);
										    	urlInput.setValue(serverData.url);
										    	document.getElementById(browseFileId).setAttribute("alt",serverData.fileId);
