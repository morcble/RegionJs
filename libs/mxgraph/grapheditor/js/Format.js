...
Format.prototype.refresh = function()
{
	...
	if (graph.isSelectionEmpty())
	{
		...
		if (this.showCloseButton)
		{
			...
			ui.actions.get('formatPanel').funct(); // 手动关闭属性栏
	...
	else
	{
		...
		var nodeType = this.getSelectionState().style.symbol; // 节点类型
		var nodeProps = [];
		// if (nodeType == 'start') { // 开始或者结束节点
		// 	nodeProps = [
		// 		{name: 'name', dispName: '名称(字母)', type: 'string'},
		// 		{name: 'wfDesc', dispName: '中文名', type: 'string'}
		// 	];
		// } else if (nodeType == 'task') { // 任务节点
		// 	nodeProps = [
		// 		{name: 'step', dispName: '子流程(字母)', type: 'string'},
		// 		{name: 'name', dispName: '子流程名'}
		// 	];
		// } else if (nodeType == 'condition') { // 条件节点
		// 	nodeProps = [
		// 		{name: ""}, {}, {}
		// 	];
		// }
		...
		// 节点属性
		mxUtils.write(label3, mxResources.get('propertyPanel'));
		div.appendChild(label3);

		var propPanel = div.cloneNode(false);
		...
		var panelBody = domParser.parseFromString(panelBodyStr, 'text/html');



		// var saveBtn = mxUtils.button("保存", mxUtils.bind(this, function(evt) {
		// 	console.log(this.editorUi.actions);
		// 	this.editorUi.actions.get('saveNodeProp').funct();

		// }));
		// saveBtn.setAttribute('title', '保存');
		// saveBtn.className += 'btnCanvas';
		// saveBtn.style.width = '100px';
		// saveBtn.style.marginBottom = '2px';
		// propPanel.appendChild(saveBtn);
		this.container.appendChild(propPanel);
		
		//addClickHandler(label2, textPanel, idx++);
		addClickHandler(label3, propPanel, idx++);
