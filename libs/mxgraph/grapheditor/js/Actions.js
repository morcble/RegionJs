...
Actions.prototype.init = function()
{
	...
	// 增加获取所有节点属性方法
	this.addAction('saveNodeProp', function(evt) {
		debugger;
	...
	this.addAction('editData...', function()
	{
		...
		var editorRegion = graph.editorRegion;


		//ui.showDataDialog(cell);
		var paraMap = new HashMap();
		paraMap.put("systemId", editorRegion.paraMap.get('systemId'));
		paraMap.put("cellId", cell.getId());
		paraMap.put("nodeType", nodeType);
		paraMap.put("nodeData", editorRegion.nodesData[`${nodeType}_${cell.getId()}`]); // 节点数据
		paraMap.put("cellLabel", cell.value);
		var tmpDef = null;
		if (nodeType == 'start') {
			tmpDef = RegionUtil.openModalWindow(RegionUtil.appendParaMapToUrl('sys/admin/rcs/wf/workflowconfig/startconfig.rs',paraMap),global_msg._edit+"工作流配置",2,0.4,0.7,1000,700);
		} else if (nodeType == 'task') {
			tmpDef = RegionUtil.openModalWindow(RegionUtil.appendParaMapToUrl('sys/admin/rcs/wf/workflowstepconfig/taskconfig.rs',paraMap),global_msg._edit+"子流程",2,null,null,1000,700);
		}
		if (!!tmpDef) {
			tmpDef.open.done(function(openRegion) {
				openRegion.editorRegion = editorRegion; // 传入外层region
				openRegion.cell = cell; // 传入节点
				openRegion.model = graph.getModel();
			});
		}
