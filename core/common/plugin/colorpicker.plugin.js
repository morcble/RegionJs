var ColorPickerPlugin = {
		render:function(region,tmpObj,paraData){
			...
			var replaceHtml = '<div class="colorpicker-wrapper region-wrapper">'+hiddenInputHtml;
			replaceHtml+='<div class="color-preview"></div>';
			replaceHtml+='<div class="color-num"></div>';
			replaceHtml+='</div>';
			...
			var colorPreview = replaced.find(".color-preview");
			if(paraData==null||paraData==""){
				paraData = "#FFFFFF";
			}
			
			if(paraData != "#FFFFFF" && paraData.toUpperCase()!="#FFFFFF")
				colorPreview.css("border-color",paraData);
			
			colorPreview.css("background-color",paraData);
			replaced.find('.color-num').text(paraData);
			...
			colorPreview.colorPicker({
			    ...
			    renderCallback: function(elm, toggled) {
			    	...
			    	if(backGroundColor.startWith("rgba")){
			    		//donothing
			    	}
			    	else{//转化为#XXXXXX
			    		var colorArray = backGroundColor.substring(4,backGroundColor.length-1).split(",");
			    		backGroundColor = "#";
			    		for(var i = 0 ; i<colorArray.length;i++){
			    			var tmpStr = parseInt(colorArray[i]).toString(16);
			    			if(tmpStr.length==1)tmpStr="0"+tmpStr;
			    			backGroundColor+= tmpStr.toUpperCase();
			    		}
			    	}
			    	
			    	var wrapped = replaced.find(".wrapped");
			    	replaced.find('.color-num').text(backGroundColor);
			    	wrapped.val(backGroundColor);
			    	
			    	if(backGroundColor=="#FFFFFF"){
			    		elm.css("border-color","lime");
			    	}
			    	else elm.css("border-color",backGroundColor);
			    	
			    	if(onchange!=null&&onchange!=""){
						eval(onchange+'.call(wrapped,backGroundColor)');
		...
		updateVal:function(region,wrapperElemObj,regionElemObj,paraData){//值改变调用此函数
			...
			var colorPreview = wrapperElemObj.find(".color-preview");
			if(paraData==null||paraData==""){
				paraData = "#FFFFFF";
			}
			...
			colorPreview.css("background-color",paraData);
			wrapperElemObj.find('.color-num').text(paraData);
		}
}
